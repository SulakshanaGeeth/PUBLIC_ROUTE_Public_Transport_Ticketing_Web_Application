...
router.put("/point/:id", async (req, res) => {
  ...
  if (obj) {
    if (
      obj.OtherPoints.filter(
        (ob) => ob.pointName == req.body.OtherPoints.pointName
      ).length == 1
    ) {
      obj.OtherPoints.filter(
        (ob) => ob.pointName == req.body.OtherPoints.pointName
      )[0].price = req.body.OtherPoints.price;
    } else {
...
router.delete("/point/:id/:point", async (req, res) => {
  ...
  if (obj) {
    obj.OtherPoints.splice(
      obj.OtherPoints.findIndex(
        (Object) => Object.pointName == req.params.point
      ),
      1
    );
